...
### 1. تجهيز بيئة wp-cli
run-hash اتأكد إنه لسه `3PbB5bi_t` (نفس المحطات 5–8). بيئة PowerShell محتاجة أيضًا إضافة مسارات `php.exe` (من `lightning-services\php-8.3.29+1\bin\win64`) و`mysql.exe` (من `lightning-services\mysql-8.4.0\bin\win64\bin`) لمتغير PATH — بدونهم `wp.bat` بيفشل بخطأ "'php' is not recognized" حتى مع PHPRC/MYSQL_HOME مضبوطين. تم فحص كود Fluent Forms لتحديد أماكن التخزين الفعلية بدل التخمين: بيانات Turnstile محفوظة في option خاص بالإعدادات العامة، مع علم منفصل يتحكم في ظهور Turnstile كخيار متاح في منشئ الفورم. تم التأكد إن الاتنين فاضيين قبل الربط. تم أيضًا فحص جدول `wp_fluentform_forms` عبر `wpdb` (مش raw `wp db query` لأن مسار MySQL socket المحلي مش متاح مباشرة لعميل mysql.exe) — الموقع فيه بس الفورمين الافتراضيين اللي بيتركبوا تلقائيًا مع Fluent Forms (`Contact Form Demo`, `Subscription Form`)، **مفيش فورم "Start a Project" حقيقي لسه** (تصميم الفورم التفصيلي لسه بند غير معتمد في `APPROVED-DECISIONS.md`) — يعني نطاق هذه المحطة هو ربط المفاتيح على مستوى الإضافة (global)، مش تفعيل الحماية على فورم فعلي بعد.
...
- إعدادات Turnstile العامة في Fluent Forms تم حفظها بدون توثيق أي قيمة حساسة في Git.
- `_fluentform_turnstile_keys_status` = `true`

اتحقق سطر-سطر إن الإعدادات المخزّنة قابلة للقراءة والاستخدام داخل WordPress، مش بس إن العملية "نجحت" بدون خطأ، وبدون كتابة أي قيمة حساسة في ملفات المشروع.
...
3. اتنفّذ `do_shortcode('[fluentform id="1"]')` فعليًا وتم فحص الـHTML الناتج: احتوى على `data-sitekey` بقيمة حقيقية مخزنة داخل قاعدة البيانات ومخفية من Git، كلاس `cf-turnstile`، وتأكد إن سكريبت Cloudflare (`turnstile`) اتسجّل كـ`enqueued` فعليًا — يعني الويدجت هيظهر فعليًا لأي زائر حقيقي لو الفورم ده كان منشور.
...
المستخدم وفّر حساب Gmail حقيقي مع طريقة مصادقة مخصصة للتطبيقات (وليس كلمة مرور الحساب الرئيسية) وطلب توثيق أن هذا اتصال **مؤقت لمرحلة التطوير/الاختبار فقط**، وأن مزوّد الإنتاج النهائي هيتحدد عند تجهيز الاستضافة الحقيقية وإيميل الدومين. لم تُسجّل أي قيمة سرية في ملفات المشروع.
...
### 4. التنفيذ الفعلي
عبر `wp eval-file`، استُدعيت `Settings::store()` مباشرة ببيانات اتصال حقيقية لكن غير موثقة في Git. تم التحقق بالقراءة الرجعية عبر `fluentMailGetSettings()` من نجاح حفظ إعدادات مزوّد التطوير ومن أن القيمة الحساسة تُفك وتُستخدم داخل WordPress فقط، بدون كتابة أي سر في ملفات المشروع.
...
`git grep` على الريبو المحلي كامل للقيم الحساسة الفعلية رجّع **صفر نتائج** — البيانات مش متتبّعة بـGit ولا مكتوبة في أي ملف من ملفات المشروع، متخزّنة فقط في `wp_options` (`fluentmail-settings`) مشفّرة.
...
**التاريخ:** 2026-07-01
**الحالة:** ✅ مكتملة
**النوع:** فحص توافق تقني + تنفيذ Polylang Free + استراتيجية Case Studies ونبرة محتوى
...
### 7. استراتيجية Case Studies ونبرة المحتوى

بعد إغلاق الجزء التقني مؤقتًا، نبّه المستخدم أن المحطة 15 لم تكتمل لأنها كانت تشمل بندًا ثانيًا من `محطات/roadmap.md`: استراتيجية Case Studies ونبرة المحتوى. تم أولًا عمل commit+push فوري لشغل Polylang الموجود:

```text
542dcc4 Complete Phase 15 localization setup
```

ثم عُرضت بوابة قرار صريحة على المستخدم بثلاثة اختيارات:

| الخيار | المعنى |
|---|---|
| Demo/Concept Projects موسومة بوضوح | عينات concept/demo واضحة وغير مدّعية لعملاء حقيقيين |
| Work "قريبًا" مؤقتًا | صفحة Work بلا case studies لحين وجود أعمال حقيقية |
| شغل شخصي حقيقي كعينات أولى | استخدام أعمال فعلية شخصية/تعليمية/تجريبية لو متاحة |

اختار المستخدم صراحةً:

```text
Demo/Concept Projects موسومة بوضوح
```

بناءً على ذلك تم تسجيل القرار رقم 25 في `APPROVED-DECISIONS.md`: يسمح ببناء عينات Demo/Concept أصلية وموسومة بوضوح في Work/Case Studies إلى حين توفر أعمال عملاء حقيقية، مع منع أي أسماء عملاء وهمية أو نتائج/شهادات غير حقيقية.

### 8. دليل النبرة الثنائي

تم إنشاء ملف مرجعي جديد:

```text
design/content/tone-of-voice.md
```

الملف يحدد نبرة Shemo Studio لمحطات بناء الصفحات 16-20:

- Hybrid studio + visible founder، مطابق للقرار 1.
- العربية هي لغة الإطلاق الأساسية، والإنجليزية counterpart طبيعي غير ترجمة حرفية.
- ثقة هادئة بدون مبالغة أو ادعاءات.
- حضور المؤسس واضح بدون تحويل الموقع لبراند شخصي بحت.
- قواعد صريحة لوسم Demo/Concept Projects:
  - عربي: `مشروع تجريبي / Concept - غير منفّذ لعميل تجاري`
  - English: `Demo / Concept Project - Not commissioned by a client`
- ممنوع: أسماء عملاء وهمية، metrics وهمية، testimonials وهمية، أو before/after claims غير حقيقية.

### 9. تحقق الإغلاق النهائي

تم تنفيذ تحقق تقني بعد اكتمال الجزأين:

- Polylang ما زال active، والافتراضي `ar`.
- `/` يرجع 200 و`lang="ar"` و`dir="rtl"`.
- `/en/` يرجع 200 و`lang="en-US"`.
- `php -l` نجح على ملفات الثيم المعدلة.
- `محطات/stages-log.html` اتعمل له HTML parse ناجح.
- `git diff --check` نجح.
- `git grep` لفحص أسرار شائعة لم يجد أسرارًا في الملفات المتتبعة.
- `MASTER-PLAN.md` بقي untracked عمدًا ولم يدخل أي commit.
...
**التاريخ:** 2026-07-01
**الحالة:** ✅ مكتملة
**النوع:** صفحات ثنائية اللغة + نموذج Start a Project فعلي + Turnstile + سياسات مسودة للمراجعة

### السياق

بدأت المحطة بعد تشغيل ذاكرة Codex المشتركة، ثم قراءة `APPROVED-DECISIONS.md` خصوصًا القرارات 7، 8، 13، 14، 20، 21، 24، 25، و`محطات/roadmap.md`، وآخر محطات 18-19 من هذا السجل، و`design/content/tone-of-voice.md`، و`git log --oneline -15`.

الواقع المؤكد قبل التنفيذ:

- الفرع `main` كان مواكبًا لـ`origin/main` على commit المحطة 19.
- يوجد ملف غير متتبع سابق `MASTER-PLAN.md` ولم يتم لمسه.
- عدد الإضافات النشطة = 15.
- Fluent Forms Free نشط بإصدار 6.2.5.
- Cloudflare Turnstile مهيأ داخل Fluent Forms بخيارات `_fluentform_turnstile_details` وحالة مفاتيح مفعلة.
- صفحة Privacy Policy القديمة ID `3` كانت `draft`.
- صفحة Complianz المنشورة Privacy Statement (EU) ID `18` كانت `publish`.
- خيار WordPress `wp_page_for_privacy_policy` كان يشير سابقًا إلى ID `3` draft.

### 1. التنفيذ

تمت إضافة سكربتين قابلين لإعادة التشغيل:

- `tools/stage20-build-start-contact-policies.php`
- `tools/stage20-verify.php`

وتمت إضافة قالبين داخل الثيم:

- `themes/shemo-child/404.php`
- `themes/shemo-child/search.php`

وأُضيف CSS داعم محدود داخل `themes/shemo-child/style.css` لصفحات السياسات والبحث و404.

تم بناء 18 صفحة منشورة داخل WordPress وربط كل زوج عبر Polylang:

| الصفحة | عربي | English |
|---|---|---|
| Start a Project | `/start-a-project/` | `/en/start-a-project-en/` |
| Contact | `/contact/` | `/en/contact-en/` |
| Thank You | `/thank-you/` | `/en/thank-you-en/` |
| Search | `/search/` | `/en/search-en/` |
| Terms | `/terms/` | `/en/terms-en/` |
| Revision Policy | `/revision-policy/` | `/en/revision-policy-en/` |
| Deposit Policy | `/deposit-policy/` | `/en/deposit-policy-en/` |
| Refund Policy | `/refund-policy/` | `/en/refund-policy-en/` |
| Delivery Policy | `/delivery-policy/` | `/en/delivery-policy-en/` |

كما تم تحديث روابط CTA الإنجليزية القديمة التي كانت تشير إلى `/en/start-a-project/` و`/en/contact/` لتشير إلى الصفحات المنشورة الفعلية.

### 2. Start a Project / Fluent Forms Free

تم إنشاء نموذجين فعليين داخل Fluent Forms Free:

| النموذج | ID | اللغة | عدد الحقول |
|---|---:|---|---:|
| Shemo Start a Project - AR | 5 | عربي | 10 |
| Shemo Start a Project - EN | 6 | English | 10 |

النموذج single-page عمدًا لحدود النسخة المجانية: بدون multi-step Pro، بدون save & resume، وبدون PDF generation. الحقول الأساسية: الاسم، البريد، البراند/المشروع، نوع المشروع، هدف المشروع، التسليمات، الميزانية، الموعد، روابط/مراجع، وعنصر Cloudflare Turnstile.

### 3. Cloudflare Turnstile

تمت إضافة عنصر Turnstile فعليًا داخل النموذجين باستخدام عنصر Fluent Forms الرسمي `turnstile` واسم الحقل `cf-turnstile-response`.

التحقق الحي المتاح على LocalWP:

- HTML صفحة `/start-a-project/` يحتوي `cf-turnstile` و`data-sitekey` وسكربت `challenges.cloudflare.com/turnstile`.
- HTML صفحة `/en/start-a-project-en/` يحتوي نفس العناصر.
- محاولة إرسال CLI كاملة الحقول بدون token حي رجعت `HTTP/1.1 422 Unprocessable Entity` مع JSON:
  - `Turnstile verification failed, please try again.`
- عدد submissions للنموذجين بقي `0` بعد الاختبار.

لم يتم اختراع نجاح إرسال حقيقي لأن توليد token Turnstile صالح يحتاج تفاعل متصفح حي مع Cloudflare widget، وليس متاحًا من اختبار CLI الآمن. ما تم إثباته فعليًا: الويدجت يظهر، السكربت الخارجي يتحمل، والتحقق server-side يمنع الإرسال بدون token.

### 4. Contact

تم بناء Contact كصفحة CTA ثنائية اللغة بدون إضافة مدفوعة وبدون نموذج إضافي جديد. الصفحة توجه حسب الحاجة إلى:
...
### 5. السياسات والخصوصية

تم بناء الصفحات القانونية المطلوبة كنصوص **مسودة قابلة للمراجعة** وليست اعتمادًا ماليًا أو قانونيًا نهائيًا:
...
كل صفحة تحتوي تنبيهًا واضحًا بأنها review draft ولا تضيف قرارًا جديدًا ولا تعتمد سياسة نهائية. لم يتم تعديل `APPROVED-DECISIONS.md`.

حل تعارض الخصوصية تم بشكل آمن:

- لم تُحذف صفحة Privacy Policy القديمة ID `3`.
- لم تُنشر صفحة Privacy Policy القديمة ولم تتحول إلى سياسة نهائية؛ بقيت `draft`.
- تم تحديث خيار WordPress `wp_page_for_privacy_policy` ليشير إلى صفحة Complianz المنشورة الحالية ID `18`، لأنها كانت منشورة بالفعل من المحطة 8.
- صفحات السياسات تشير إلى Privacy Statement (EU) وCookie Policy (EU) المنشورتين من Complianz، مع توثيق أن الصفحة القديمة draft باقية للمراجعة.

### 6. تحقق WordPress / Polylang / DB

نجح `tools/stage20-verify.php` وأكد:

- كل الصفحات التسع لها زوج عربي/إنجليزي منشور.
- كل زوج مربوط في الاتجاهين عبر Polylang.
- Rank Math description موجود لكل صفحة.
- نموذجا Start a Project منشوران ويحتويان عنصر Turnstile.
- Turnstile configured داخل Fluent Forms.
- `wp_page_for_privacy_policy=18`.
- `legacy_privacy_status=draft`.
- عدد الإضافات النشطة = 15.
...
- `/start-a-project/` و`/en/start-a-project-en/`
- `/contact/` و`/en/contact-en/`
- `/thank-you/` و`/en/thank-you-en/`
- `/search/` و`/en/search-en/`
- `/terms/` و`/en/terms-en/`
- `/revision-policy/` و`/en/revision-policy-en/`
- `/deposit-policy/` و`/en/deposit-policy-en/`
- `/refund-policy/` و`/en/refund-policy-en/`
- `/delivery-policy/` و`/en/delivery-policy-en/`
- نتائج البحث `/?s=motion` و`/en/?s=motion`

قالب 404 تم اختباره على `/stage20-missing-page-check/` ورجع `404` كما هو متوقع لصفحة غير موجودة، بدون redirect.

### 8. فحوص الإغلاق

- `php -l` نجح على سكربتات المحطة وقوالب `404.php` و`search.php` باستخدام PHP الخاص بـLocalWP.
- لا يوجد `debug.log` قبل/بعد الاختبارات، وبالتالي لم تُضف أسطر debug جديدة.
- عدد الإضافات النشطة = 15.
- فحص الروابط القديمة داخل محتوى الصفحات أكد:
  - `/en/start-a-project/ count=0`
  - `/en/contact/ count=0`
- فحص الأسرار عبر `rg` رجع مطابقات نصية غير سرية مثل أسماء options (`secretKey`) وتوثيق قديم، بدون قيم مفاتيح أو tokens مكشوفة.
- `git diff --check` نجح مع تحذير CRLF المعروف فقط.
- `phpcs`/WPCS غير متاح في PATH، لذلك لم يمكن تشغيله في هذه البيئة.
